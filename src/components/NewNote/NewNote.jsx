...
export function NewNote({ onClose, dataNote }) {
  ...
  const confirmAction = () => {
    if(!dataNote.id){
      ...
      editNote(dataNote.id, title, note);
    }
    onClose();
  }
    ...
    <>
      <div className="flex flex-col items-center p-2 border-2 border-gray-700 rounded-xl  w-72 h-72 ">
        <form className="flex flex-col">
          ...
          <button type='submit' onClick={confirmAction} className="self-end mt-4 mr-2 rounded-xl bg-gray-900 text-white text-sm p-1 w-12">
            Save
          </button>
        </form>
      </div>
